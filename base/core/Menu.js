...
//           style={{ width: this.props.labelWidth || "inherit" }}
//         >
//           {this.props.labelDom ? this.props.labelDom : this.props.label}
...
//     cursor: default;
//     outline: none;
//     padding: ${props => (props.labelDom ? "0" : "3px 15px 4px")};
...
class Dropdown extends Component {
  constructor(props) {
    ...
    this.wrapperRef = React.createRef(); // <-- REPLACES findDOMNode
    this.handleDocumentClick = this.handleDocumentClick.bind(this);
  }
  ...
  /** Replaces your old populate logic */
  populate = props => {
    if (!props) return;

    if (props.value) {
      const selected = props.options?.find(
        item => item.value.toString() === props.value
      );
  ...
  handleDocumentClick(event) {
    // SAFE ALTERNATIVE TO findDOMNode
  ...
  setValue = (value, label) => {
    ...
    if (this.props.onChange) this.props.onChange(newState.selected);
    this.setState(newState);
  };
  ...
  renderOption(option) {
    ...
    return (
      <div
        key={option.value}
        className={optionClass}
        onMouseDown={() => this.setValue(option.value, option.label)}
      >
        {option.label}
      </div>
    );
  ...
  render() {
    ...
    return (
      <div
        className={`${baseClassName}-root ${isOpen ? "is-open" : ""}`}
        ref={this.wrapperRef} // <-- SAFELY CONTAIN COMPONENT
      >
        <div
          className={`${baseClassName}-control`}
          onMouseDown={this.handleMouseDown}
        >
          <div className={`${baseClassName}-placeholder`}>
            {selected.label}
          </div>
        </div>
